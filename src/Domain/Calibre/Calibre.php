...
class Calibre implements CalibreRepository
{
    ...
    public function authorsSlice($index = 0, $length = 100, $search = null, $translit=false): array
    {
    ...
    public function authorsInitials()
    {
        ...
        foreach ($initials as $initial) {
            $i = new Item();
            $ctr = $this->findOne(Item::class, 'SELECT COUNT(*) as ctr FROM authors WHERE substr(upper(sort),1,1)=:initial', ['initial' => $initial->initial]);
    ...
    # Return an array with elements: current page, no. of pages, $length entries
    public function tagsSlice($index = 0, $length = 100, $search = null, $translit = false)
    {
    ...
    public function tagsInitials()
    {
        ...
        foreach ($initials as $initial) {
            $i = new Item();
            $ctr = $this->findOne(Item::class, 'SELECT COUNT(*) as ctr FROM tags WHERE substr(upper(name),1,1)=:initial', ['initial' => $initial->initial]);
            $i->initial = $initial->initial;
            $i->ctr = $ctr->ctr;
            array_push($ret, $i);
        }
    ...
    public function titlesSlice($lang, $index=0, $length=100, $filter = null, $search = null, $translit = false)
    {
    ...
    public function seriesInitials()
    {
        ...
        foreach ($initials as $initial) {
            $i = new Item();
            $ctr = $this->findOne(Item::class, 'SELECT COUNT(*) as ctr FROM series WHERE substr(upper(name),1,1)=:initial', ['initial' => $initial->initial]);
            $i->initial = $initial->initial;
            $i->ctr = $ctr->ctr;
            array_push($ret, $i);
        }
